...
def test_check2__has_stopporders_completed():
    assert d.query("status=='Completed' & type=='Stop' ").shape[0] > 0
...
def test_check6__no_rejected_orders():
    Rejected = d.query("status=='Rejected'")
    assert Rejected.shape[0] == 0, "There were rejected orders, strategy might be invalid!"
